# What is nectarblocks?

> **This plugin is not affiliated with ThemeNectar or the [NectarBlocks](https://themenectar.com) product.**
> It is a developer tool built specifically to support teams who use NectarBlocks and need a structured workflow for creating their own custom blocks alongside it.

nectarblocks is a Claude Code plugin for building custom WordPress blocks using ACF (Advanced Custom Fields) and the Gutenberg block editor — designed to complement the NectarBlocks ecosystem, not replace it.

It is not a page builder. It does not add pre-built blocks to your site. It is a **development workflow** — a set of conventions, tools, and AI-powered commands that help developers build custom blocks consistently and efficiently alongside NectarBlocks.
...
**An expert agent** — the `nectarblocks-dev` agent has full context of the workflow built in. No explaining the stack every session.
